...
async def post_creation(msg: Message, state: FSMContext):
    ...
    if len(data) == 1:
        ...
        await state.update_data(new_post=data[0])
...
async def edit_post(msg: Message):
    scheduler.add_job(send_message, "date", run_date=datetime(2023, 11, 5, 22, 53), args=(data, msg))
async def send_message(data, msg):
    bot = Bot(token=config.BOT_TOKEN, parse_mode=ParseMode.HTML)
    await bot.send_message(data['new_post'], msg.text)
